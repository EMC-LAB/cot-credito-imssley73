...
document.addEventListener("DOMContentLoaded", function () {
    ...
    formularioSimulador.addEventListener("input", function (evento) {
        ...
        const mapaErrores = {
            montoDeseado: "errorMonto",
            edad: "errorEdad",
            plazoCredito: "errorPlazo",
            pensionBruta: "errorPension"
        };

        const idError = mapaErrores[campo.id];
        const error = idError ? document.getElementById(idError) : null;
...
function limpiarErrores() {
    ...
    document.querySelectorAll(".mensaje-error").forEach(function (elemento) {
        elemento.textContent = "";
    });

    document.querySelectorAll(
        "#formSimulador input, #formSimulador select"
...
function procesarSimulacion(evento) {
    ...
    const hayErrores = validarDatosSimulador({
        montoDeseado,
        edad,
        plazo,
        pensionMensual
    ...
    if (hayErrores) {
        const primerCampoConError =
            document.querySelector("#formSimulador .input-error");
...
function validarDatosSimulador(datos) {
    ...
    if (!montoDeseado) {
        mostrarError(
            "montoDeseado",
            "errorMonto",
            "Ingresa el monto que necesitas."
        );
        hayErrores = true;
    } else if (montoDeseado < 6000) {
        mostrarError(
            "montoDeseado",
            "errorMonto",
            "El monto mínimo disponible es de $6,000."
        );
        hayErrores = true;
    } else if (montoDeseado > 600000) {
        mostrarError(
            "montoDeseado",
            "errorMonto",
            "El monto máximo disponible es de $600,000."
        );
        hayErrores = true;
    }
    ...
    if (!edad) {
        ...
        mostrarError(
            "edad",
            "errorEdad",
            "La edad mínima para solicitar un crédito es de 60 años."
        );
        hayErrores = true;
    } else if (edad > 74) {
        mostrarError(
            "edad",
            "errorEdad",
            "La edad máxima para solicitar un crédito es de 74 años."
        );
        hayErrores = true;
    }

    if (!CONFIGURACION_CREDITO.plazosPermitidos.includes(plazo)) {
        mostrarError(
            "plazoCredito",
            "errorPlazo",
            "Selecciona un plazo válido para continuar."
        );
        hayErrores = true;
    }
    ...
    if (!pensionMensual || pensionMensual <= 0) {
        mostrarError(
            "pensionBruta",
            "errorPension",
            "Ingresa una cantidad válida para tu pensión mensual."
        );
        hayErrores = true;
    }
